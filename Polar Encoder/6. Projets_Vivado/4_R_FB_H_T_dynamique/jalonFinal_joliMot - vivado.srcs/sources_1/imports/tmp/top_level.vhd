...
entity toplevel is
	...
	port(
		btn1,btn0,clk, RxD: in std_logic;	--supprimer btn1 aussi sur contraintes
		TxD: out std_logic;
		outputMsg, outputMsg2 : out std_logic_vector(0 to 7)
	);
end entity ;
...
architecture test of toplevel is
	...
	signal Send_reception, SendFbits, SendHadamard, SendTransmit, Sread_transmit_enable : std_logic:='0';
	signal SstartFbits, SstartHadamard: std_logic:='0';
	signal Sstart_reception: std_logic:='1';
	...
	signal Sdata_in,sortie_test : std_logic_vector (0 to 7) := (others=>'0');-- Entree RAM  reception
	
	--Caracterstiques message entrée et message de sortie
	signal Sk : integer:=0;
	signal Sn : integer:=0;
	signal SlogM : integer:=0;

begin

--L_test : test_reception port map (clk, Sout_RAMr, SstartFbits, Sk, SraddLRAM, sortie_test);
L_UART_RX : UART_RX generic map (10416) port map (clk,Sstart_reception , RxD, Send_reception, SWEReception, Sdata_in,SraddERAM, Sk,Sn,SlogM);
Lcontrole : controle port map(clk, btn0,Send_reception, SendFbits, SendHadamard, SendTransmit, Sstart_reception, SstartFbits, SstartHadamard, SstartTransmission, SresetFbits, SresetHadamard);
L_Fbits	  : fbits port map (clk,SresetFbits,SstartFbits,Sout_RAMe_Fb,Sout_RAMr,Sk,Sn,SlogM,SendFbits,Sin_RAMe_Fb,SWEEncodage_Fb,Sereset,SaddERAM_Fb,SaddLRAM_Fb,SraddLRAM);
